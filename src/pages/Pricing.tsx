...
const CollapsibleFAQCard: React.FC<{ 
  faq: { question: string; answer: string };
  isExpanded: boolean;
  onToggle: () => void;
}> = ({ faq, isExpanded, onToggle }) => {
  return (
    <div className={`relative bg-gray-900 rounded-2xl border transition-all duration-500 ease-in-out transform ${
      isExpanded 
        ? 'border-cyan-500/50 shadow-lg shadow-cyan-500/10 scale-[1.01]' 
        : 'border-gray-700/50 hover:border-cyan-500/30 hover:scale-[1.005]'
    }`}>
      {/* Collapsible Button Header */}
      <button
        onClick={onToggle}
        className={`w-full p-6 text-left transition-all duration-300 ease-in-out rounded-t-2xl flex items-center justify-between ${
          isExpanded 
            ? 'bg-gradient-to-r from-cyan-500/5 to-blue-500/5 border-b border-cyan-500/20' 
        ...
        <div className="flex-shrink-0">
          <div className={`transition-all duration-300 ${isExpanded ? 'rotate-180' : 'rotate-0'}`}>
            {isExpanded ? (
              ...
              <ChevronDown className="h-5 w-5 text-gray-400 group-hover:text-cyan-400" />
            )}
          </div>
        </div>
      </button>
...
const CollapsiblePackageCard: React.FC<{ 
  pkg: Package; 
  type: 'website' | 'app';
  isExpanded: boolean;
  onToggle: () => void;
}> = ({ pkg, type, isExpanded, onToggle }) => {
  ...
  return (
    <div className={`relative bg-gray-900 rounded-2xl border transition-all duration-500 ease-in-out transform ${
    ...
    } ${isExpanded ? 'scale-[1.02] shadow-2xl shadow-cyan-500/10' : 'hover:scale-[1.01]'}`}>
      {pkg.popular && (
      ...
      <button
        ...
        className={`w-full p-6 text-left transition-all duration-300 ease-in-out rounded-t-2xl flex items-center justify-between ${
          isExpanded 
            ? 'bg-gradient-to-r from-cyan-500/10 to-blue-500/10 border-b border-cyan-500/20' 
      ...
      >
        <div className="flex-1">
          ...
          <div className={`text-3xl font-bold mb-2 transition-colors duration-300 ${
            isExpanded ? 'text-cyan-300' : 'text-cyan-400'
          }`}>{pkg.price}</div>
        ...
        <div className="ml-4 flex-shrink-0">
          <div className={`transition-all duration-300 ${isExpanded ? 'rotate-180' : 'rotate-0'}`}>
            {isExpanded ? (
              ...
              <ChevronDown className="h-6 w-6 text-gray-400 group-hover:text-cyan-400" />
            )}
          </div>
        </div>
      </button>
      ...
      <div className={`overflow-hidden transition-all duration-500 ease-in-out ${
        ...
        <div className="px-6 pb-6 border-t border-gray-700/50">
          <div className={`pt-6 transform transition-all duration-500 ease-in-out ${
            isExpanded ? 'translate-y-0 opacity-100' : 'translate-y-4 opacity-0'
          }`}>
            <h4 className="text-lg font-semibold text-white mb-4">Caratteristiche incluse:</h4>
            <ul className="space-y-3 mb-6">
              {pkg.features.map((feature, index) => (
                <li 
                  key={index} 
                  className={`flex items-start space-x-3 transform transition-all duration-300 ease-in-out ${
...
export const Pricing: React.FC = () => {
  const [expandedWebsitePackage, setExpandedWebsitePackage] = useState<number | null>(null);
  const [expandedAppPackage, setExpandedAppPackage] = useState<number | null>(null);
  const [expandedFAQ, setExpandedFAQ] = useState<number | null>(null);

  const toggleWebsitePackage = (index: number) => {
    setExpandedWebsitePackage(expandedWebsitePackage === index ? null : index);
  };

  const toggleAppPackage = (index: number) => {
    setExpandedAppPackage(expandedAppPackage === index ? null : index);
  };
          ...
          <div className="grid gap-8 max-w-4xl mx-auto">
            {websitePackages.map((pkg, index) => (
              <CollapsiblePackageCard
                ...
                pkg={pkg}
                type="website"
                isExpanded={expandedWebsitePackage === index}
                onToggle={() => toggleWebsitePackage(index)}
              />
            ))}
          </div>
        </section>
          ...
          <div className="grid gap-8 max-w-4xl mx-auto">
            {appPackages.map((pkg, index) => (
              <CollapsiblePackageCard
                ...
                pkg={pkg}
                type="app"
                isExpanded={expandedAppPackage === index}
                onToggle={() => toggleAppPackage(index)}
              />
            ))}
          </div>
        </section>
